...
class MemberService
{
    ...
    {
        if ($member) {
            $member->loadMissing(['creator.person', 'person.contacts', 'person.user', 'branch', 'subscriptions.plan.planActivities.staffActivity.activity', 'subscriptions.items', 'healthProfile', 'measurements']);
        }
        return $member;
    }
