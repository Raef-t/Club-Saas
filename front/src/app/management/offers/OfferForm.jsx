...
import { Field, CheckboxField } from "@/components/forms/FormControls";
...
export default function OfferForm({
  ...
  const activityTypeOptions = useMemo(() => {
    ...
    return [
      { value: "all", label: "جميع أنواع الأنشطة" },
      ...uniqueOptions,
    ];
  }, [rawActivityTypes]);
  ...
  const { data: plansData, isLoading: isPlansLoading } =
    useGetSubscriptionPlansQuery(branchParams);
      ...
      if (selectedActivityTypeId !== "all") {
        ...
        const matchesActivityType =
          ...
          (Array.isArray(plan.activity_types) &&
            plan.activity_types.some((at) => 
              String(at.id) === String(selectedActivityTypeId) ||
              (targetKey && normalizeArabicText(formatLocalizedName(at.name) || at.name) === targetKey)
            )) ||
          ...
          (Array.isArray(plan.activities) &&
            plan.activities.some((act) => {
              ...
              const typeName = formatLocalizedName(act.activity_type?.name) || act.activity_type?.name;
              return (
          ...
          (plan.activity_type && (
            ...
            (targetKey && normalizeArabicText(formatLocalizedName(plan.activity_type.name) || plan.activity_type.name) === targetKey)
          ));
  ...
  }, [allPlans, selectedActivityTypeId, activityTypeOptions]);
  ...
  const singleChoiceSavingsMin = minRegularPrice > offerPriceNum ? minRegularPrice - offerPriceNum : 0;
  const singleChoiceSavingsMax = maxRegularPrice > offerPriceNum ? maxRegularPrice - offerPriceNum : 0;
  const singleChoiceDiscountPercent = avgRegularPrice > 0 && singleChoiceSavingsMin > 0
    ? Math.round(((avgRegularPrice - offerPriceNum) / avgRegularPrice) * 100)
    : 0;

  // Bundle savings (total package savings & per-activity savings)
  const bundleTotalSavings = totalRegularPrice > offerPriceNum ? totalRegularPrice - offerPriceNum : 0;
  const bundleSavingsPerPlan = planCount > 0 && bundleTotalSavings > 0
    ? Math.round(bundleTotalSavings / planCount)
    : 0;
  const bundleDiscountPercent = totalRegularPrice > 0 && bundleTotalSavings > 0
  ...
  return (
    <form id={formId} onSubmit={handleSubmit} className="space-y-4 text-right" dir="rtl" noValidate>
      ...
      <div className="space-y-2 pt-1">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <span className="text-sm text-app-muted-light">
            الفعاليات المشمولة في العرض *
          </span>
          {form.plans.length > 0 && (
        ...
        {errors.plans && (
          <p className="text-xs text-app-red">{errors.plans}</p>
        )}

        {isPlansLoading ? (
          <p className="py-4 text-center text-xs text-app-muted-light">جاري تحميل الفعاليات...</p>
          ...
          <div className="max-h-56 overflow-y-auto space-y-1.5 rounded-lg border border-app-line bg-app-card-soft/40 p-2">
            {filteredPlans.map((plan) => {
              ...
              const isUnlimited = Boolean(
                plan.is_unlimited_subscribers || !plan.max_subscribers || Number(plan.max_subscribers) === 0
              );
              const slots = plan.available_slots !== undefined && plan.available_slots !== null
                ...
                >
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      checked={isSelected}
                      onChange={() => {}}
                      className="size-4 rounded border-app-line text-app-yellow focus:ring-0 cursor-pointer"
                    />
                    <div className="text-right">
                      ...
                      <div className="mt-0.5">
                        {isUnlimited ? (
                          <span className="text-[10px] text-app-muted-light">المقاعد: غير محدود</span>
                        ) : slots <= 0 ? (
                          <span className="text-[10px] text-app-red font-medium">مكتمل العدد (0/{plan.max_subscribers})</span>
                        ) : (
                ...
                {bundleTotalSavings > 0 ? (
                  <>
                    <div className="flex items-center justify-between text-emerald-400 font-medium border-t border-app-line/50 pt-1">
                      ...
                      <span>{formatMoney(bundleTotalSavings)} (خصم {bundleDiscountPercent}%)</span>
                    </div>
                    ...
                    {planCount > 1 && (
                      <div className="flex items-center justify-between text-app-muted-light text-[11px]">
                        ...
                        <span className="text-white font-medium">{formatMoney(bundleSavingsPerPlan)}</span>
                      </div>
                    )}
                  </>
                ) : (
      ...
      <div className="rounded-xl border border-app-line bg-app-card-soft/80 p-3.5 text-xs space-y-2">
        <div className="flex items-center justify-between">
          ...
          >
            {offerAvailableCapacity ? offerAvailableCapacity.label : "يُحسب تلقائياً حسب الفعاليات المحددة"}
          </span>
        </div>
        ...
        {offerAvailableCapacity?.type === "single_choice" && offerAvailableCapacity.perPlan && (
          <div className="border-t border-app-line/60 pt-2 space-y-1.5">
            <span className="text-[11px] text-app-muted-light block">المقاعد المتاحة لكل فعالية:</span>
        ...
        {offerAvailableCapacity?.type === "bundle" && !offerAvailableCapacity.isUnlimited && (
          <p className="text-[11px] text-app-muted-light">
            (في الباقة: تعتمد السعة على الفعالية المحدودة، أو الفعالية الأقل مقاعد min عند وجود أكثر من فعالية محدودة)
          </p>
        )}
      </div>
      ...
      {showFooterActions && (
        <div className="flex gap-3 pt-4 border-t border-app-line">
